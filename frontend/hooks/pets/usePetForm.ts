...
export function usePetForm(navigation: NavigationProp<RootStackParamList>) {
  const [pet, setPet] = useState({ petName: '', petWeight: '', petBreed: '', petSpecies: '', petDietaryRestrictions: [] as string[] });
  ...
  const handleSubmit = () => {
    if (!isOnline) {
      enqueue({ type: 'createPet', payload: { name: pet.petName.trim(), weight: parseFloat(pet.petWeight.trim()), breed: pet.petBreed, species: pet.petSpecies, dietaryRestrictions: pet.petDietaryRestrictions } });
      showToast('No connection — pet will sync when back online', 'error');
      return;
    }

    createPet(
      { name: pet.petName.trim(), weight: parseFloat(pet.petWeight.trim()), breed: pet.petBreed, species: pet.petSpecies, dietaryRestrictions: pet.petDietaryRestrictions },
      {
